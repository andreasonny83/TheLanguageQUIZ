...
class DbHandler {
	...
	public function getUserProfile( $userUID ) {
		$stmt = $this->conn->prepare( 'SELECT
			username, email
			FROM LQ_users WHERE user_uid = ? LIMIT 1' );
		$stmt->bind_param( 's', $userUID );
	...
	public function updateUserProfile( $userUID, $username, $email ) {
		if ( $stmt = $this->conn->prepare( 'UPDATE LQ_users
			...
			$stmt->bind_param( 'sss', $username, $email, $userUID );
			$stmt->execute();
			$stmt->close();
	...
	public function updateUserPassword( $userUID, $email, $old_password, $password, $password_confirm ) {
		if ( ! $db_uid = $this->checkUserPassword( $email, $old_password ) ) {
			return false;
		}
	...
	public function updateUserAvatar( $avatar, $user_uid ) {
		// Update user
		$stmt = $this->conn->prepare( "UPDATE LQ_users_data
			SET avatar = ?
			WHERE user_uid = ?" );
